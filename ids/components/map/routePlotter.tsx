...
export function RoutePlanner({ map }: { map: L.Map | null }) {
    ...
    const { dequeuedRoute, clearQueue } = useRoutePlanner()
    ...
    useEffect(() => {
        if (dequeuedRoute) {
            ...
            // If it's an FAA route, remove first and last 4 characters
            if (dequeuedRoute.source === "faa") {
                processedRoute = processedRoute.slice(4, -4)
            }
    ...
    const handleAddRouteWithString = async (routeString: string) => {
        ...
        try {
            const response = await fetch(
                `/api/nav/route?route=${encodeURIComponent(routeString.trim())}`
            )
